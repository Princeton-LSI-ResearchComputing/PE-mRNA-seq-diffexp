...
PE6max.vs.PEmax.mut_editor.diffexp <-
...
PE6max.vs.PEmax.mut_editor.genes <-
  PE6max.vs.PEmax.mut_editor.diffexp %>%
...
x <- list(
  "PEmax vs PE6max Mutant" = PE6max.vs.PEmax.mut_editor.genes,
  "PE6max vs PE6max Mutant" = PE6max.vs.PE6max.mut_editor.genes,
  "PEmax vs PE6max" = PE6max.vs.PEmax_editor.genes
)
...
```{r}
PE6max.vs.PEmax.mut_editor.maplot <- ggpubr::ggmaplot(
  PE6max.vs.PEmax.mut_editor.diffexp,
  fdr = 0.05,
  size = 0.5,
  ylim = c(-1, 1),
  fc = 1,
  genenames = PE6max.vs.PEmax.mut_editor.diffexp$gene,
  top = 0,
  main = "HEK3: PEmax vs PE6max Mutant"
) +
  ggplot2::theme(legend.position = c(0.1, 0.9)) # +

PE6max.vs.PEmax.mut_editor.maplot

ggplot2::ggsave("PE6max.vs.PEmax.mut_editor.maplot.pdf")
```
