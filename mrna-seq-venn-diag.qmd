...
for (contrast in contrasts) {
  ...
  gene.counts[[contrast]] <- sapply(gene.lists, length)
...
```
```{r, fig.width=20}
pdf(file.path(output_dir, "venn-diagrams-to-scale.pdf"))
venn.diags.scaled <- plot(euler(c("a" = 1, "b" = 23, "a&b" = 1, "c" = 90, "d" = 182, "c&d" = 22, "e" = 95, "f" = 35, "e&f" = 5)), quantities = TRUE)
plot(venn.diags.scaled)
dev.off()
plot(venn.diags.scaled)
```
